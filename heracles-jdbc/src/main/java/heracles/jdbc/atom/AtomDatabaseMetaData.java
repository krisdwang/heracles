...
public class AtomDatabaseMetaData implements DatabaseMetaData {

	private static final Logger LOGGER = LoggerFactory.getLogger(AtomDatabaseMetaData.class);

	private DatabaseMetaData databaseMetaData;

	public AtomDatabaseMetaData(DataSource dataSource) throws SQLException {
		Connection conn = null;
		try {
			conn = DataSourceUtils.getConnection(dataSource);
			this.databaseMetaData = conn.getMetaData();
		} catch (SQLException e) {
			LOGGER.error("AtomDatabaseMetaData: init AtomDatabaseMetaData error : " + e.getMessage());
