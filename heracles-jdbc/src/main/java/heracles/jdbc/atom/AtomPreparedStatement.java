...
public class AtomPreparedStatement extends AtomStatement implements PreparedStatement {

	private static final Logger LOGGER = LoggerFactory.getLogger(AtomPreparedStatement.class);
	...
	public AtomPreparedStatement(AtomConnection atomConnection) throws SQLException {
		super(atomConnection);
	}
	...
	public boolean execute() throws SQLException {
		...
		case UPDATE:
			...
			return false;
		default:
			throw new SQLParserException("AtomPreparedStatement execute:not support this sql : " + sql
	...
	public ResultSet executeQuery() throws SQLException {
		...
		parser.eval(parameterSettings, ShardingType.TABLE);
		String[] targetSqls = parser.getShardingSQLs();
		Assert.notEmpty(targetSqls);

		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("AtomPreparedStatement executeQuery: this sql split to " + targetSqls.length);
		}

		List<ResultSet> resultSets = new ArrayList<ResultSet>();
		try {
			Connection conn = atomConnection.getTargetConnection();
			conn.setAutoCommit(isAutoCommit());

			for (String targetSql : targetSqls) {
				PreparedStatement targetPreparedStatement = _prepareStatement(conn, targetSql);
				setParameters(targetPreparedStatement);
				getAtomConnection().setTargetConnection(conn);
				getAtomConnection().getTargetStatements().add(targetPreparedStatement);
				ResultSet resultSet = targetPreparedStatement.executeQuery();
				attachedResultSets.add(resultSet);
				resultSets.add(resultSet);

				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("AtomPreparedStatement executeQuery: the targetSql is [" + targetSql
	...
	public int executeUpdate() throws SQLException {
		...
		try {
			...
			if (ArrayUtils.isNotEmpty(targetSqls)) {
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("AtomPreparedStatement executeUpdate: this sql split to " + targetSqls.length);
				}
				for (String targetSql : targetSqls) {
					if (targetPreparedStatement == null) {
						targetPreparedStatement = _prepareStatement(conn, targetSql);
					}
					this.setParameters(targetPreparedStatement);
					targetPreparedStatement.addBatch();
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("AtomPreparedStatement executeUpdate: the targetSql is [" + targetSql + "]");
					}
				}
				atomConnection.setTargetConnection(conn);
				atomConnection.getTargetStatements().add(targetPreparedStatement);

				int[] rows = targetPreparedStatement.executeBatch();
